...
scale calibrated to the 96 in drum shell; see xd96_silhouette.json.  Everything else is a block estimate.
...
STA={'DB':(-12,150),'XD':(156,580),'FB':(586,688),'BH':(694,801),'BU':(813,951)}
BLK=[  # tag, name, x, y, w, h
 ('D-1','Dewatering unit + feed tank\n(dewatered digestate)',26,50,20,14),
 ('P-1','Dried product bunker / bagging',82,50,20,14),
 ('C-1','Cyclone\n+ airlock',92,33,8,8),
 ('F-1','Fan + stack',102,34,8,6),
 ('BGS','Biogas conditioning skid',10,8,24,10),
 ('CH','Chiller',36,10,6,8),
 ('MCC','MCC / control room',48,6,16,12),
 ('LPG','LPG vaporizer / regulator station\n(by others)',104,6,10,10),
]
train_ft2=sum((b-a)/144.0*2 for _,a,b in plan)  # each 2-pt station step = 2*2.991 in
AREAS=[
 ('Dryer train: burner and fuel train, burner housing, feed breeching, 8 ft x 40 ft drum, drop-out box (from drawing 104400900)',round(train_len*train_w),'%.0f ft long x %.0f ft wide overall; drop-out box top 17 ft, drum centreline 8.4 ft above grade'%(train_len,train_w)),
...
c.setStrokeColor(GREY); c.setLineWidth(0.5); c.setDash(2,2); c.rect(X(2),Y(19),110*SC,30*SC); c.setDash()
# blocks
...
c.rect(X(tx(s1)),Y(YC-55/12),(s1-s0)/12*SC,110/12*SC,fill=1)
...
arrow(c,37,50,37,42.5); label(c,38,46,'cake')
route(c,[(84,45.5),(84,47),(86,47),(86,50)],RED); label(c,87,47.5,'dry product')
route(c,[(89,41),(90,41),(90,37),(92,37)],RED); label(c,89.5,43,'exhaust')
arrow(c,100,37,102,37); arrow(c,110,40,110,46); label(c,111,43,'stack')
arrow(c,96,41,96,50); label(c,97,45,'fines')
...
route(c,[(104,13),(100,13),(100,26),(17,26),(17,30)],ORANGE); label(c,50,27,'LPG vapor, 5 psig',ORANGE)
arrow(c,64,12,66,12); label(c,65,9,'480 V feeder / cable tray to all drives',GREY)
...
c.drawString(X(104),Y(20),'Truck access this side'); c.drawString(X(104),Y(3.5),'LPG tanks: outside pad, min. 50 ft from dryer')
...
for h,t in ((0,'grade'),(101,'drum centreline 8.4 ft'),(157,'burner housing top 13.1 ft'),(205,'drop-out box top 17.1 ft')):
...
c.setFillColor(NAVY); c.setFont('Helvetica-Bold',7); c.drawString(ex0,ey0-12,'burner / fuel train'); c.drawString(EX(760),ey0-22,'burner housing'); c.drawString(EX(500),ey0-12,'drum on base frame, two trunnion stations'); c.drawString(EX(150),ey0-22,'drop-out box')
...
rows.append([Paragraph('<b>Equipment pad incl. clearances and access</b>',base),Paragraph('<b>%s</b>'%format(pad_ft2,','),base),Paragraph('<b>%.0f</b>'%(pad_ft2*FT2M2),base),Paragraph('%d ft x %d ft; roofed area approx. 3,000 sq ft (280 sq m) over dryer train and dewatering'%PAD,base)])
rows.append([Paragraph('<b>Recommended site allowance</b>',base),Paragraph('<b>14,500</b>',base),Paragraph('<b>1,350</b>',base),Paragraph('approx. 45 m x 30 m: pad, truck loop, LPG tank farm set-back, digestate piping corridor',base)])
